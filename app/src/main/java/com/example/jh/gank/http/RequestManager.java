...
import com.example.jh.gank.db.DBManager;
import com.example.jh.gank.utils.NetworkUtils;
...
public class RequestManager {
     ...
     * @param callBack
     */
    public static void get(Object tag, final String url, final boolean isCache, final CallBack<List<GanHuo>> callBack) {
        final DBManager dbManager = new DBManager();
        String data = dbManager.getData(url);
        if(!"".equals(data)){
            // 解析json数据并返回成功回调
            callBack.onSuccess(new Gson().<List<GanHuo>>fromJson(data, callBack.type));
        }
